...
for (const runtimeName of [setupRuntimeName, backupRuntimeName]) {
...
if (index.includes('/phase7-first-time-setup.js') || index.includes('/phase7-backup-bridge.js')) {
...
console.log('linked Phase 7 setup and backup runtimes after Phase 2 and immediately before protected app.js');

const serviceWorkerPath = resolve(dist, 'service-worker.js');
let serviceWorker = await readFile(serviceWorkerPath, 'utf8');
if (serviceWorker.includes('/phase7-first-time-setup.js') || serviceWorker.includes('/phase7-backup-bridge.js')) {
...
console.log('linked both Phase 7 subscriber assets into the deployed service-worker cache');
